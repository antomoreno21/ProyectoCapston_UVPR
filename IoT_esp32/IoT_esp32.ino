...
void loop() {
  ...
  if ((Serial.available() > 0) /*&& (digitalRead(22) == HIGH)*/)
  {
    mensajeArduino = Serial.readString();
    //Serial.println();
    //Serial.print();
  }
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  //Prueba de retorno
  int valorMQTT;
  // Quitar cuando se terminen las pruebas si se necesita
  for (int i = 0; i < length; i++) {  // Se imprime y concatena el mensaje
    //Serial.print((char)message[i]);
    messageTemp += (char)message[i];
  }
  valorMQTT = messageTemp.toInt();
  ...
  if (String(topic) == "esp32/output") {  // En caso de recibirse mensaje en el tema esp32/output
    ...
    Serial.println(valorMQTT);
    /*if(/*messageTemp // valorMQTT >= 1){
      //Serial.println("Led encendido");
      //digitalWrite(ledPin2, HIGH);
    }// fin del if (String(topic) == "esp32/output")
    else if(/*messageTemp // valorMQTT <= 0){
